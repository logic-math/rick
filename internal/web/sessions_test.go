...
// --- worker crash closes the session ---

func TestSessionWorkerDeathClosesSession(t *testing.T) {
	env := newTestEnv(t)
	m := env.managerWith(t, nil, nil)
	...
	entry := waitForStatus(t, env, id, SessionStatusClosed)
	if entry.Status != SessionStatusClosed {
		t.Fatalf("crashed worker session: %q", entry.Status)
	}
}
