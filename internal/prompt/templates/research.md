...
## SENSE 各步骤简报格式(research 输出给 sense_loop)

所有步骤简报均引用主报告的"尽调树快照"+ 节点详情,然后追加步骤特定追问:

| 步骤 | 简报追加内容(在尽调树快照后) |
|---|---|
| S1 | R7 上报项 + ① 现状补充? ② 期望? ③ 差距? |
| E1 | human 确认后写入 `{{loop_dir}}/briefs/E1.md`(替代原 loops.md) |
| E2 | 视角 A/B(基于叶节点事实)+ 事实无法区分处 + human 选择? |
| N | 主要矛盾候选表(含来源节点+置信度)+ R7 上报项 + human 判断? |
| S-R | 阻碍 + 逆转逻辑 + 替代路径(均基于叶节点事实)+ human 判断? |
