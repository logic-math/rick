...
1. **Domain 加载（优先）**：读取 `{{domain_dir}}/` 下所有 `.md` 文件，提取与本次需求相关的事实（已知问题、构建命令、环境配置、接口规范等）。这些事实将直接影响任务分解和风险预判。

2. **Loops 上下文初始化**：读取上方 `{{loops_context}}` 中列出的项目已有 loop 模式，了解项目积累的工作流经验；如有相关 loop 文件，读取触发条件与范围，判断本次需求是否匹配某个已有 loop
