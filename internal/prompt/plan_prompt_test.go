...
func TestGeneratePlanPrompt_NoOKRInfo(t *testing.T) {
	...
	// Create plan.md template (no {{okr_content}} - OKR is now job-level, generated by Claude)
...
**用户需求**: {{user_requirement}}`
	...
	// Verify prompt contains default SPEC message (no OKR in plan prompt)
...
func TestGeneratePlanPrompt_FormatCorrect(t *testing.T) {
	...
	// Create plan.md template (no {{okr_content}} - OKR is now job-level)
	planTemplate := `# 规划提示词
