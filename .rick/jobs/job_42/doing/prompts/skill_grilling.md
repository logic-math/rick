...
> ⚠️ 本文件是 grilling 的**唯一编排协议源**（设计树模型 + 下钻循环 + 调研分工 + research 派发 + 追问规范）。plan / easy / doing 只注入本 skill 的路径与各自的落盘目录（`/workdir/sunquan20/rick-dev/.rick/jobs/job_42/doing/grilling`），**不各自维护编排逻辑**——所有阶段以本文件为准执行。
...
```text
subagent({ workflowScript: "return runs.run('research-L{N}', { agent: 'research', task: '<调研清单：当前层不可自行消解的事实性问题，逐条澄清到高置信度（附来源/信源等级）。简报落盘：/workdir/sunquan20/rick-dev/.rick/jobs/job_42/doing/grilling/research-L{N}.md（write 首块 + bash 分批追加）；最终回复=回执（路径+要点 ≤300 字）>' })", timeoutMs: 3600000 })
```
...
- `/workdir/sunquan20/rick-dev/.rick/jobs/job_42/doing/grilling/design-tree.md` —— 活文档，逐层追加（层号 + 模块 + pipeline + 判断节点）
- `/workdir/sunquan20/rick-dev/.rick/jobs/job_42/doing/grilling/research-L{N}.md` —— L1 的 research 简报（凡该层存在不可自查消解的问题就必须有）
- 对 human 的批量追问（L3）—— 一层至少一轮（若该层零判断节点，须在 design-tree.md 显式记录「本层全部消解，无判断节点」并说明依据）

```
for each layer (top-down):
    L1 调研消解：轻量自查 + 重量级派 agent:'research'（简报落盘 /workdir/sunquan20/rick-dev/.rick/jobs/job_42/doing/grilling/research-L{N}.md）
...
```text
grilling_gate 工具：{ grilling_dir: "/workdir/sunquan20/rick-dev/.rick/jobs/job_42/doing/grilling" }
```
