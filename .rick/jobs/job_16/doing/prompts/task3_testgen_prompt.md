...
**Task ID**: task3
**Task Name**: 更新 Go prompt 文件，将 super-debugging-zh 切换到 debug_skill
**Task Goal**: 修改 `doing_prompt.go`、`plan_prompt.go`、`easy_prompt.go` 中所有 WriteSkillFile / SetVariable 调用，将 "super-debugging-zh" 文件名和对应模板变量名全部切换为 "debug_skill"/"debug_skill_path"；同步更新 `manager_test.go` 相关测试用例，确保 `go test ./internal/prompt/...` 全部通过。
...
请创建测试脚本到: `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_16/doing/tests/task3.py`
...
- 脚本应该可以直接运行: `python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_16/doing/tests/task3.py`
...
```bash
python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_16/doing/tests/task3.py
```
