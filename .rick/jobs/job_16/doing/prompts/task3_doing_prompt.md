...
**任务 ID**: task3
**任务名称**: 更新 Go prompt 文件，将 super-debugging-zh 切换到 debug_skill
...
2. `WriteSkillFile(promptsDir, "skill_super_debugging_zh.md", "super-debugging-zh")` → `WriteSkillFile(promptsDir, "skill_debug_skill.md", "debug_skill")`（三个参数必须同步，filename 用于磁盘，skillName 用于 embed 检索）
3. `SetVariable("super_debugging_path", ...)` → `SetVariable("debug_skill_path", filepath.Join(promptsDir, "skill_debug_skill.md"))`
4. **新增**：`WriteSkillFile(promptsDir, "skill_sense.md", "sense")` —— debug_skill.md 中 review debug agent 需要加载 sense skill，必须确保该文件在 doing prompts 目录中存在
5. **新增**：`SetVariable("sense_skill_path", filepath.Join(promptsDir, "skill_sense.md"))` —— 供 debug_skill.md 模板引用 `{{sense_skill_path}}`
6. 错误消息（`WriteSkillFile` 失败提示）中的旧文件名同步更新
7. `internal/prompt/plan_prompt.go`：
8. `SetVariable("super_debugging_skill_path", ...)` → `SetVariable("debug_skill_path", ...)`
9. dry-run 占位值从 `"<doing-prompts>/skill_super_debugging_zh.md"` → `"<doing-prompts>/skill_debug_skill.md"`
10. 对应变量（`superDebuggingSkillPath`）重命名为 `debugSkillPath`，路径从 `skill_super_debugging_zh.md` → `skill_debug_skill.md`
11. `internal/prompt/easy_prompt.go`：
12. `WriteSkillFile` 和 `SetVariable` 参数同 doing_prompt.go 同步更新（变量名必须统一为 `debug_skill_path`）
13. **新增**：`WriteSkillFile(promptsDir, "skill_sense.md", "sense")` 和 `SetVariable("sense_skill_path", ...)` —— 与 doing_prompt.go 保持一致
14. 读 easy.md 模板确认其引用的占位符名称，确保 SetVariable 的 key 与模板中 `{{变量名}}` 完全一致
15. `internal/prompt/manager_test.go`：
16. 将测试列表中的 `"super-debugging-zh"` 替换为 `"debug_skill"`
17. 确认 LoadCoreSkills([]string{"debug_skill"}) 返回非空内容（依赖 task1 已完成且文件存在于 embed）
18. `go build ./...` 无编译错误
19. `go test ./internal/prompt/...` 全部通过
...
```
2026/06/11 15:41:54 warn: core skill not found: templates/skills/nonexistent-skill-xyz.md (open templates/skills/nonexistent-skill-xyz.md: file does not exist)
...
FAIL
FAIL	github.com/sunquan/rick/internal/prompt	0.190s
FAIL



Full test output:
{"pass": false, "errors": ["doing_prompt.go still contains old pattern: 'super-debugging-zh'", "doing_prompt.go still contains old pattern: 'super_debugging_zh'", "doing_prompt.go still contains old pattern: 'super_debugging_path'", "doing_prompt.go still contains old pattern: 'super-debugging'", "plan_prompt.go still contains old pattern: 'super_debugging_zh'", "plan_prompt.go still contains old pattern: 'super_debugging_skill_path'", "easy_prompt.go still contains old pattern: 'super-debugging-zh'", "easy_prompt.go still contains old pattern: 'super_debugging_zh'", "easy_prompt.go still contains old pattern: 'super_debugging_path'", "easy_prompt.go still contains old pattern: 'super-debugging'", "doing_prompt.go missing 'debug_skill'/'debug-skill' reference", "easy_prompt.go missing 'debug_skill'/'debug-skill' reference", "plan_prompt.go missing 'debug_skill_path' reference", "manager_test.go still contains old pattern: 'super-debugging-zh'", "manager_test.go still contains old pattern: 'super-debugging'", "manager_test.go missing 'debug_skill'/'debug-skill' reference", "go test ./internal/prompt/... FAILED:\n2026/06/11 15:41:54 warn: core skill not found: templates/skills/super-debugging-zh.md (open templates/skills/super-debugging-zh.md: file does not exist)\n--- FAIL: TestIntegration_RFC001 (0.00s)\n    --- FAIL: TestIntegration_RFC001/task3/doing_prompt_no_tools_section_when_no_tools (0.00s)\n        integration_rfc001_test.go:164: GenerateDoingPromptFile: failed to write super-debugging-zh skill: embedded skill \"super-debugging-zh\" not found\n2026/06/11 15:41:54 warn: core skill not found: templates/skills/super-debugging-zh.md (open templates/skills/super-debugging-zh.md: file does not exist)\n2026/06/11 15:41:54 warn: core skill not found: templates/skills/nonexistent-skill-xyz.md (open templates/skills/nonexistent-skill-xyz.md: file does not exist)\n--- FAIL: TestCoreSkillsEmbed (0.00s)\n    --- FAIL: TestCoreSkillsEmbed/all_eight_skill_files_non_empty (0.00s)\n        manager_test.go:203: LoadCoreSkills(\"super-debugging-zh\") returned empty string\n--- FAIL: TestGeneratePlanPrompt_NoUnreplacedVars (0.00s)\n    plan_prompt_test.go:80: Expected all template variables to be replaced\nFAIL\nFAIL\tgithub.com/sunquan/rick/internal/prompt\t0.190s\nFAIL\n\n"]}
