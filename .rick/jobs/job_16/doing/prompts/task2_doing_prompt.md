...
**任务 ID**: task2
**任务名称**: 更新 doing.md / plan.md 模板引用，删除 super-debugging-zh.md
...
3. 第 3 行声明由 `skill:super-debugging` → `skill:debug-skill`
4. `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_16/doing/prompts/skill_super_debugging_zh.md` 替换为 `{{debug_skill_path}}`（出现在 core skills 列表和 DEBUG 铁律章节）
5. 在 core skills 列表中**新增一行** `- skill:sense（系统化思维，供 review debug agent 使用）：{{sense_skill_path}}`
6. DEBUG 铁律章节：将 "I will use skill:super-debugging." 声明 + 五阶段流程描述替换为 "I will use skill:debug-skill." + 三阶段调试流程（加载 debug_skill 文件后执行）
7. 工作日志规范章节的 `debug{N}:` 详细问题记录格式整体删除，改为一句：`遇到 bug 时，加载并严格遵循 skill:debug-skill，在 debug/ 目录下按 bug{n}-{描述}.md 格式记录`
8. 保留 `task{N}:` 执行日志格式（分析过程/实现步骤/遇到的问题（写"无"或引用 debug/{file}）/验证结果）不变
9. `internal/prompt/templates/plan.md` 更新：
10. `{{super_debugging_skill_path}}` 替换为 `{{debug_skill_path}}`
11. 将 "super-debugging skill" 改为 "debug-skill"
12. 将执行顺序说明从 "S（还原问题）→ E（视角分析）→ N（验证假设）→ 修复实现 → 3 次失败则停止找人类协作者" 改为 "按 debug-skill 三阶段调试法（源码推理法→增量调试法→科学实验法）执行，每阶段达上限后升级人工协作"
13. 以上所有变更在 `git diff` 中可见，`git grep super_debugging internal/prompt/templates/` 返回空
...
```
=== Attempt 2 ===
test did not pass: internal/prompt/templates/skills/super-debugging-zh.md 仍存在，应已删除; 模板目录中仍有旧引用 (super_debugging/super-debugging): internal/prompt/templates/doing.md
internal/prompt/templates/easy.md
internal/prompt/templates/plan.md
internal/prompt/templates/skills/super-debugging-zh.md; doing.md 缺少 {{debug_skill_path}} 占位符; doing.md 缺少 skill:debug-skill 声明; plan.md 缺少 {{debug_skill_path}} 占位符; doing.md 中旧的 debug{N} 格式未清理; doing.md 缺少 {{sense_skill_path}} 占位符

Full test output:
{"pass": false, "errors": ["internal/prompt/templates/skills/super-debugging-zh.md 仍存在，应已删除", "模板目录中仍有旧引用 (super_debugging/super-debugging): internal/prompt/templates/doing.md\ninternal/prompt/templates/easy.md\ninternal/prompt/templates/plan.md\ninternal/prompt/templates/skills/super-debugging-zh.md", "doing.md 缺少 {{debug_skill_path}} 占位符", "doing.md 缺少 skill:debug-skill 声明", "plan.md 缺少 {{debug_skill_path}} 占位符", "doing.md 中旧的 debug{N} 格式未清理", "doing.md 缺少 {{sense_skill_path}} 占位符"]}
