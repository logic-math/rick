...
**Task ID**: task8
**Task Name**: 在 learning_check 和 dream_check 中集成 loops/skills 格式校验
**Task Goal**: 在 `internal/cmd/` 包中新建共享校验文件 `tools_loops_skills_check.go`，实现 `runLoopsAndSkillsCheck(rickDir string) []string` 函数，校验 `.rick/loops/` 和 `.rick/skills/` 目录下所有候选文件的格式合规性，并分别在 `runLearningCheck()` 和 `runDreamCheck()` 末尾调用，确保 learning/dream 阶段写出的候选文件在 check 时被一并校验。
...
**正常路径 - loops/skills 格式正确时 learning_check 通过**：
...
**异常路径 - loop 文件缺 trigger 时 learning_check 报错**：
...
清理：删除 bad_loop.md
**异常路径 - skill 文件缺章节时 dream_check 报错**：
...
请创建测试脚本到: `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_22/doing/tests/task8.py`
...
- 脚本应该可以直接运行: `python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_22/doing/tests/task8.py`
...
```bash
python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_22/doing/tests/task8.py
```
