...
**任务 ID**: task6
**任务名称**: 迁移 learning prompt builder + learning.md 模板：移除 wiki/tools 输出，改写为 loops/skills 产出
...
4. `builder.SetVariable("loops_context", LoadLoopsContext(loopsDir))`（loopsDir 由 `workspace.GetRickDir()` 推导）
5. `builder.SetVariable("loops_dir", loopsDir)`
6. `builder.SetVariable("skills_dir", skillsDir)`（skillsDir = `{rickDir}/skills/`）
7. `go test ./internal/prompt/... -run TestLearning` 全部通过（或相关 embedded 测试通过）
8. `./bin/rick learning --job job_22 --dry-run 2>&1` 包含 "loops_dir" 和 "skills_dir" 路径，不包含 "wiki_dir" 或 "spec_path"
9. `learning.md` 模板中写完候选文件后的 check 步骤只需调用 `rick tools learning_check {{job_id}}`，不需要单独调用 loops/skills check（task8 已将其集成进 learning_check）
...
```
=== Attempt 5 ===
test did not pass: dry-run output missing resolved 'loops_dir' path; dry-run output missing resolved 'skills_dir' path

Full test output:
{"pass": false, "errors": ["dry-run output missing resolved 'loops_dir' path", "dry-run output missing resolved 'skills_dir' path"]}
