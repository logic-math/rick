...
**任务 ID**: task7
**任务名称**: 迁移 easy + dream prompt builders + 模板：移除 SPEC/OKR/wiki/tools，注入 loops_context
...
` 的注入（仅这两个，**`{{debug_content}}` 必须保留**，它是当前 session 的 debug 记录，与 SPEC/OKR 无关）
...
```
=== Attempt 3 ===
test did not pass: easy --dry-run output missing '可用的项目 Loops' section

Full test output:
{"pass": false, "errors": ["easy --dry-run output missing '可用的项目 Loops' section"]}
