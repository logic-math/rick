...
**Task ID**: task2
**Task Name**: 将 `debug_skill.md` 三阶段结构替换为 diagnosing-bugs Phase 1-6
**Task Goal**: 用 RFC-001 中 diagnosing-bugs 的 Phase 1-6 抽象（构建反馈回路→复现最小化→可证伪假设→插桩观察→修复回归→清理事后分析）替换 `internal/prompt/templates/skills/debug_skill.md` 中现有的三阶段结构（源码推理法/增量调试法/科学实验法），保留 bug*.md frontmatter 格式规范和 review debug agent 协议，并重新构建二进制。
...
**边界用例 - 直接读取 skill 文件验证 Phase 结构**（注意：debug_skill.md 通过 WriteSkillFileWithVars 写出为独立文件，不在 doing dry-run 主体输出中，须直接读取源文件验证）：
...
请创建测试脚本到: `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_22/doing/tests/task2.py`
...
- 脚本应该可以直接运行: `python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_22/doing/tests/task2.py`
...
```bash
python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_22/doing/tests/task2.py
```
