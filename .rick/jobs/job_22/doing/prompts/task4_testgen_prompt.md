...
**Task ID**: task4
**Task Name**: 迁移 doing prompt builder + doing.md 模板：移除 SPEC/OKR，注入 loops_context
**Task Goal**: 修改 `internal/prompt/doing_prompt.go` 和 `internal/prompt/templates/doing.md`：
...
` 和 `{{job_okr_content}}` 变量，添加 `{{loops_context}}` 变量
4. 更新相关单元测试
...
预期输出：包含 "可用的项目 Loops"（loops_context 注入成功），不包含 "# SPEC
...
| mark-task-success | doing_check 因 tasks.json status 非 success 失败 | .rick/wiki/mark_task_success_workflow.md |
| adding-new-skill-to-templates | 向 plan/easy 模板注入新 skill（WriteSkillFile 模式） | .rick/wiki/adding_new_skill_to_templates.md |
" 或 "{{job_okr_content}}"，且包含 "loops_context" 相关内容
...
请创建测试脚本到: `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_22/doing/tests/task4.py`
...
- 脚本应该可以直接运行: `python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_22/doing/tests/task4.py`
...
```bash
python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_22/doing/tests/task4.py
```
