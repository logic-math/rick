...
**Task ID**: task3
**Task Name**: 实现 `LoadLoopsContext()` 函数，遍历 `.rick/loops/` 提取触发点列表
**Task Goal**: 在 `internal/prompt/manager.go`（或 `context_helpers.go`）中实现 `LoadLoopsContext(loopsDir string) string` 函数，遍历指定目录下所有 `*.md` 文件，解析 YAML frontmatter 提取 `trigger` 字段，拼接为格式化的 loops 触发列表字符串，供各 phase prompt builder 调用。
...
请创建测试脚本到: `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_22/doing/tests/task3.py`
...
- 脚本应该可以直接运行: `python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_22/doing/tests/task3.py`
...
```bash
python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_22/doing/tests/task3.py
```
