...
**任务 ID**: task8
**任务名称**: 在 learning_check 和 dream_check 中集成 loops/skills 格式校验
...
5. .rick/loops/*.md: frontmatter (name, trigger) + 5 sections (目标/上下文管理/可调用工具/产出评估/停止标准)
6. .rick/skills/*.md: frontmatter (name, description) + 4 sections (When to Use/Procedure/Pitfalls/Verification)
7. README.md in each dir is skipped
8. `NewDreamCheckCmd()` 的 Long description 同步更新，补充相同的 loops/skills 校验说明
9. `tools.go` 的 Long description 中 learning_check 和 dream_check 的行内说明更新，提及 loops/skills 格式校验
10. `go test ./internal/cmd/... -run TestLoopsSkillsCheck` 全部通过
11. `./scripts/build.sh` 成功
