...
路径：`/workdir/sunquan20/AI_CODING/rick/.rick/domain`
...
设计 `# 测试方法` 章节时，必须参考：
- skill:tdd（路径：`/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/plan/prompts/skill_tdd_zh.md`）— 确保先写测试、覆盖红绿循环
- skill:testing-anti-patterns（路径：`/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/plan/prompts/skill_testing_anti_patterns_zh.md`）— 避免测试反模式
...
```
/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/plan
```

- 任务文件：`/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/plan/task1.md`、`task2.md` 等
...
将/workdir/sunquan20/AI_CODING/rick/.rick/draft/rfc/rfc-rick-三层架构重构与spec信息内 核-2026-08-14.md 这个 RFC 的三个O 最终落地达成目标

---

## ⚠️ 必须严格按以下 9 步 SOP 执行，不可跳过任何一步

1. **Domain 加载（优先）**：读取 `/workdir/sunquan20/AI_CODING/rick/.rick/domain/` 下所有 `.md` 文件，提取与本次需求相关的事实（已知问题、构建命令、环境配置、接口规范等）。这些事实将直接影响任务分解和风险预判。
...
4. **grilling 追问**：加载 skill:grilling（路径：`/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/plan/prompts/skill_grilling.md`），对用户需求逐问追问，给出推荐答案，将需求澄清到具体可落实的代码路径或工具调用级别，达到终止条件后再继续
...
7. **六维评审**（每个 subagent 独立启动，**串行执行**，上一个完成后再启动下一个）：
   ...
   - subagent_5：测试用例完整性 —— 参考 skill:tdd（`/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/plan/prompts/skill_tdd_zh.md`）和 skill:testing-anti-patterns（`/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/plan/prompts/skill_testing_anti_patterns_zh.md`），检查每个 task 的测试方法是否覆盖四要素（前置条件/输入参数/操作序列/预期输出），同时验证无测试反模式（如测试 mock 行为、仅用于测试的生产方法等）
   - subagent_6：端到端验证设计 —— 以用户视角设计可复用的验收测试方法：明确用户操作入口、预期的可观测输出、异常路径的兜底验证，确保交付产物质量可被客观检验
   - 每个 subagent 输出评审结论后，根据结论修正 task 文件，再启动下一个

8. **格式检查**：运行 `/workdir/sunquan20/AI_CODING/rick/bin/rick tools plan_check job_35`；失败则修复后重新运行，直至通过

9. **输出**：按 task.md 格式保存到 `/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/plan/task{N}.md`
