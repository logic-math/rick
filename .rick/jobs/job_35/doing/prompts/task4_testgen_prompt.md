...
**Task ID**: task4
**Task Name**: 重构 runtime 模块（pi 调用逻辑收口到 runtime 层）
**Task Goal**: 按 spec（task2）落地 KR2.4：把 pi 调用逻辑（参数解析 + 调用）收口到 runtime 层。将 `internal/agent/piagent` 迁移为 `internal/runtime`（pi 调用封装）。
...
请创建测试脚本到: `/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/doing/tests/task4.py`
...
- 脚本应该可以直接运行: `python3 /workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/doing/tests/task4.py`
...
```bash
python3 /workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/doing/tests/task4.py
```
