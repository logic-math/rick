...
**Task ID**: task9
**Task Name**: 注册 think/research/exporter 为 pi 自定义 agent（经 env 职责 3 落盘）
**Task Goal**: 按 spec（task2）落地 KR3.2 的 agent 注册部分：将 think/research/exporter 注册为 pi 自定义 agent——frontmatter 落盘到 `~/.rick/pi/agent/agents/`（user 级），system prompt = 对应源码 skill 的 wiki 内容，tools 声明对齐 pi 工具名。此任务产出的是 env 职责 3（`DeployRickCustomizations`）中「agent」定制物的具体内容。
...
请创建测试脚本到: `/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/doing/tests/task9.py`
...
- 脚本应该可以直接运行: `python3 /workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/doing/tests/task9.py`
...
```bash
python3 /workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/doing/tests/task9.py
```
