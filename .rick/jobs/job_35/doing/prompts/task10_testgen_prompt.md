...
**Task ID**: task10
**Task Name**: 让 pibuilder 产出单文件内聚的 pi 定制化规范产物
**Task Goal**: 按 spec（task2）落地 KR3.1：pibuilder 产出 pi 定制化规范产物——pi agent 运行时所需的所有提示词内聚在单文件内，被 pi 以标准规范化定制开发语言引用使用。将「主 prompt 文件 + 多个 skill 文件散落（按路径引用）」改为「单文件内聚」：每个 cmd 的 pibuilder 产物把 skill/loop 内容内联进单个 .md 产物（而非散落文件 + 路径引用），可被 pi 直接加载执行。
...
请创建测试脚本到: `/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/doing/tests/task10.py`
...
- 脚本应该可以直接运行: `python3 /workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/doing/tests/task10.py`
...
```bash
python3 /workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/doing/tests/task10.py
```
