...
**Task ID**: task1
**Task Name**: 定义 spec 规范与「spec → 开发计划 → 功能等价实现」验收标准
**Task Goal**: 升级 rick 项目的 domain 描述方法，新增「spec」概念（KR1.1 + KR1.3）。spec = 结构化自然语言描述的工程实现契约。产出 spec 规范文档：定义 spec 的结构模板（四要素：模块边界 / 职责 / 接口契约 / 验收标准）、spec 与 domain 其他文档的关系，以及 spec 的验收标准定义（spec → 开发计划 → 功能等价实现；功能等价 = 通过所有功能验收，即「丢弃源码即可 AI coding 出功能等价的 rick」）。
...
请创建测试脚本到: `/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/doing/tests/task1.py`
...
- 脚本应该可以直接运行: `python3 /workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/doing/tests/task1.py`
...
```bash
python3 /workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_35/doing/tests/task1.py
```
