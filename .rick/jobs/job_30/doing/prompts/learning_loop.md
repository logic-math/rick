...
读取 `/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_30/doing/prompts/skill_gen_skill.md`，按其定义的格式（触发场景 / 预期效果 / 核心内容）从 act-path 和 debug 中提取可复用技能：
...
读取 `/workdir/sunquan20/AI_CODING/rick/.rick/jobs/job_30/doing/prompts/skill_gen_loop.md`，从 act-path 和 debug 中识别 job 内反复出现的循环模式，按其定义的完整格式写入 `/workdir/sunquan20/AI_CODING/rick/.rick/loops/{name}-loop.md`：
