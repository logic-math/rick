...
- skill:tdd（测试驱动开发）：`/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_26/doing/prompts/skill_tdd_zh.md`
- skill:testing-anti-patterns（测试反模式）：`/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_26/doing/prompts/skill_testing_anti_patterns_zh.md`
...
**Task ID**: task4
**Task Name**: 升级 learning 阶段：注入 draft_dir 变量并添加 domain 事实同步到 draft/progress.md 步骤
**Task Goal**: 在 `internal/prompt/templates/learning.md` 中添加 `{{draft_dir}}` 变量和"Draft 同步"可选步骤（如 draft/ 存在，将本次 job 产出的关键 domain 事实追加到 `draft/progress.md`）；在 `internal/cmd/learning.go` 的 `buildLearningPrompt()` 中注入 `draft_dir`（通过 `workspace.GetDraftDir()`），同时在 `runLearningDryRun()` 中注入占位路径。
...
请创建测试脚本到: `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_26/doing/tests/task4.py`
...
- 脚本应该可以直接运行: `python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_26/doing/tests/task4.py`
...
```bash
python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_26/doing/tests/task4.py
```
