...
**Task ID**: task2
**Task Name**: 升级 think agent 模板：每个 SENSE 阶段结束时捕获关键判断到 judgment.md，Perspective 阶段写入 draft/loops.md
**Task Goal**: 修改 `internal/prompt/templates/human_loop_think.md`，增加"判断记录协议"章节：在每个 SENSE 阶段的推进条件满足后，提取 1-3 条关键判断（保留原话）写入 `{{draft_dir}}/human-learning/judgment.md`；在 Perspective 阶段识别出值得展开的概念节点时，将建议写入 `{{draft_dir}}/loops.md`（四字段结构）。
...
请创建测试脚本到: `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_26/doing/tests/task2.py`
...
- 脚本应该可以直接运行: `python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_26/doing/tests/task2.py`
...
```bash
python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_26/doing/tests/task2.py
```
