...
**Task ID**: task3
**Task Name**: 升级 express agent 模板：添加 judgment.md review 清洗步骤和 ZPD 显式评价引导
**Task Goal**: 修改 `internal/prompt/templates/human_loop_express.md`，在现有 Step 1 之前增加"第零步：judgment.md review"（读取并清洗 judgment.md，删除无效/混乱条目），在现有 Step 4（文档审核）之后增加"第五步：ZPD 显式评价"（引导 human 显式评价本次 loop 的难度/收获，将结果写入 `{{draft_dir}}/progress.md`，并将下次建议写入 `{{draft_dir}}/loops.md`）。
...
请创建测试脚本到: `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_26/doing/tests/task3.py`
...
- 脚本应该可以直接运行: `python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_26/doing/tests/task3.py`
...
```bash
python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_26/doing/tests/task3.py
```
