...
/workdir/sunquan20/rick-dev/.rick/jobs/job_41/doing/debug
...
加载并**完整执行 skill:grilling**（唯一编排协议源：OKR 设计树动态下钻五步循环 + 调研分工 + research 派发 + 追问规范——一切以其为准，本段不重复协议内容）：`/workdir/sunquan20/rick-dev/.rick/jobs/job_41/doing/prompts/skill_grilling.md`

**执行锚点（防漂移）**：先 read 该 skill 全文；**必须按 L1→L5 loop 逐步推进**——第一动作 = 建立设计树根层（O + KR 集）并落盘。
**Grilling 结束后**，将澄清结论追加到 `/workdir/sunquan20/rick-dev/.rick/jobs/job_41/doing/requirement.md`（只追加，不替换）。
...
1. **优先搜索 `/workdir/sunquan20/rick-dev/.rick/domain/bugs.md` 和 `/workdir/sunquan20/rick-dev/.rick/domain/`**，查看是否有精确解决方案
   - 有匹配 → 直接应用，记录引用来源
   - 无匹配 → 继续下方流程
2. 声明：`"I will use skill:debug-skill."`，加载 skill 文件：`/workdir/sunquan20/rick-dev/.rick/jobs/job_41/doing/prompts/skill_debug_skill.md`
...
`/workdir/sunquan20/rick-dev/.rick/jobs/job_41/doing/prompts/learning_loop.md`
