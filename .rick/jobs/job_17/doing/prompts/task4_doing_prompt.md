...
- skill:tdd（测试驱动开发）：`/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/prompts/skill_tdd_zh.md`
- skill:testing-anti-patterns（测试反模式）：`/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/prompts/skill_testing_anti_patterns_zh.md`
- skill:debug-skill（调试技能）：`/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/prompts/skill_debug_skill.md`
- skill:sense（系统化思维，供 review debug agent 使用）：`/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/prompts/skill_sense.md`
...
**任务 ID**: task4
**任务名称**: 更新 SPEC 和 wiki 文档，清理 rick easy 独立命令引用和 tools merge 残留引用
...
### 测试方法
**SPEC 无 tools merge 相关内容**
操作：`grep -n "tools merge\|rick merge\|RFC-005" .rick/SPEC.md`
预期输出：无输出（0 匹配）
**wiki 文档无 tools merge / RFC-005 残留**
操作：`grep -rn "tools merge\|RFC-005" .rick/wiki/ --include="*.md"`
预期输出：无输出（0 匹配）
**OKR KR2.2 已更新，且 learning_phase_workflow.md 有人工合并说明**
...
2. 在 `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/debug/` 下创建 `bug{N}-{描述}.md`，**严格按以下格式**（doing_check 逐行校验，格式错误 = check 失败）：
...
3. 加载 `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/prompts/skill_debug_skill.md`，严格按三阶段执行（阶段一上限 3 次，阶段三上限 5 次）
...
**Immediately after test failure: 声明 "I will use skill:debug-skill."，在 `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/debug/` 创建 `bug{N}-{描述}.md`，按阶段一：源码推理法 → 阶段二：增量调试法 → 阶段三：科学实验法 顺序调试，不可跳过。**
...
2. **bug 强制记录**: 每次测试失败，必须在 `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/debug/bug{N}-{描述}.md` 创建调试记录，不可跳过
...
4. **强制 doing check**: 在 git commit 之后，**必须**运行以下命令验证产出：
   ```bash
   /Users/sunquan/ai_coding/CODING/rick/bin/rick tools doing_check job_17
   ```
...
```
=== Attempt 1 ===
test did not pass: SPEC.md 仍含 'tools merge' 引用，需清理; SPEC.md 仍含 'RFC-005' 引用，需清理; wiki/ 中仍含 'tools merge' 引用:
/Users/sunquan/ai_coding/CODING/rick/.rick/wiki/learning_phase_workflow.md:45:5. 手动将各产出文件合并到 `.rick/`（`rick tools merge` 尚未实现，见 RFC-005）
/Users/sunquan/ai_coding/CODING/rick/.rick/wiki/rick_tools_commands.md:75:- **`rick tools merge` 尚未实现**（见 RFC-005），知识合并当前需人工手动 `git merge`; wiki/ 中仍含 'RFC-005' 引用:
/Users/sunquan/ai_coding/CODING/rick/.rick/wiki/learning_phase_workflow.md:45:5. 手动将各产出文件合并到 `.rick/`（`rick tools merge` 尚未实现，见 RFC-005）
/Users/sunquan/ai_coding/CODING/rick/.rick/wiki/rick_tools_commands.md:75:- **`rick tools merge` 尚未实现**（见 RFC-005），知识合并当前需人工手动 `git merge`; OKR KR2.2 仍含 'rick tools merge': - KR2.2: `rick tools merge` 实现 learning 产出到 `.rick/` 的安全合并，分支隔离 + 人工审核双重保障; learning_phase_workflow.md 仍含 RFC-005 引用，需清理

Full test output:
{"pass": false, "errors": ["SPEC.md 仍含 'tools merge' 引用，需清理", "SPEC.md 仍含 'RFC-005' 引用，需清理", "wiki/ 中仍含 'tools merge' 引用:\n/Users/sunquan/ai_coding/CODING/rick/.rick/wiki/learning_phase_workflow.md:45:5. 手动将各产出文件合并到 `.rick/`（`rick tools merge` 尚未实现，见 RFC-005）\n/Users/sunquan/ai_coding/CODING/rick/.rick/wiki/rick_tools_commands.md:75:- **`rick tools merge` 尚未实现**（见 RFC-005），知识合并当前需人工手动 `git merge`", "wiki/ 中仍含 'RFC-005' 引用:\n/Users/sunquan/ai_coding/CODING/rick/.rick/wiki/learning_phase_workflow.md:45:5. 手动将各产出文件合并到 `.rick/`（`rick tools merge` 尚未实现，见 RFC-005）\n/Users/sunquan/ai_coding/CODING/rick/.rick/wiki/rick_tools_commands.md:75:- **`rick tools merge` 尚未实现**（见 RFC-005），知识合并当前需人工手动 `git merge`", "OKR KR2.2 仍含 'rick tools merge': - KR2.2: `rick tools merge` 实现 learning 产出到 `.rick/` 的安全合并，分支隔离 + 人工审核双重保障", "learning_phase_workflow.md 仍含 RFC-005 引用，需清理"]}
