...
**Task ID**: task4
**Task Name**: 更新 SPEC 和 wiki 文档，清理 rick easy 独立命令引用和 tools merge 残留引用
**Task Goal**: 更新 `.rick/SPEC.md`、`.rick/wiki/` 下所有文档，删除或修正：① `rick easy`（独立命令形式）的描述（`rick doing --easy` 功能仍存在，只删除将其描述为独立命令的部分）；② `rick tools merge` "尚未实现"引用（merge 功能已决策不做）；③ OKR 中 KR2.2 的 merge 相关描述。确保文档与实现现状一致。
...
请创建测试脚本到: `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/tests/task4.py`
...
- 脚本应该可以直接运行: `python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/tests/task4.py`
...
```bash
python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/tests/task4.py
```
