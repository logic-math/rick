...
**任务 ID**: task3
**任务名称**: 重构 easy.go 消除内部重复，复用已有 callClaudeCodeCLI
...
3. `callClaudeCodeCLIEasy(cfg, sessionID, promptFile)` → `callClaudeCodeCLI(cfg, promptFile, "--session-id", sessionID)`（promptFile 非空，正常追加）
4. `callClaudeCodeCLIResume(cfg, sessionID)` → `callClaudeCodeCLI(cfg, "", "--resume", sessionID)`（promptFile 为空，不追加，最终命令为 `claude --resume <sessionID>`，与原行为完全一致）
5. `go build ./...` 通过，无未使用 import、无未定义符号
6. `go test ./internal/cmd/...` 通过
...
预期输出：无输出（0 匹配）
**参数顺序校验（防止 sessionID 与 promptFile 互换）**
