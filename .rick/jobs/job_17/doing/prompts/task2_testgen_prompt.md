...
**Task ID**: task2
**Task Name**: 提取公共 frontmatter 解析函数到 internal/parser 包
**Task Goal**: 在 `internal/parser/frontmatter.go` 中实现 `ExtractBugFrontmatter(content string) (summary, status string)` 导出函数，消除 `internal/executor/debug_dir.go` 和 `internal/prompt/easy_prompt.go` 中两份相同的私有 frontmatter 解析逻辑重复。
...
请创建测试脚本到: `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/tests/task2.py`
...
- 脚本应该可以直接运行: `python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/tests/task2.py`
...
```bash
python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_17/doing/tests/task2.py
```
