...
**任务 ID**: task1
**任务名称**: 合并 tc.md 内容到 tdd-zh.md 并删除死代码 skill 文件
...
```
=== Attempt 1 ===
test did not pass: tdd-zh.md 合并内容不足：关键词命中 0/6，期望 >=5（缺失：['测试用例四要素', '前置条件', '输入参数', '操作序列', '预期输出', 'INSUFFICIENT_FUNDS']）; tdd-zh.md 缺少四要素子节标题: ['### 前置条件', '### 输入参数', '### 操作序列', '### 预期输出']; 死代码文件未删除: internal/prompt/templates/skills/tc.md; 死代码文件未删除: internal/prompt/templates/skills/tdd.md; 死代码文件未删除: internal/prompt/templates/skills/tdd/testing-anti-patterns.md; manager_test.go 仍包含已删除 skill 引用: "tc"; manager_test.go 仍包含已删除 skill 引用: "tdd"; manager_test.go 仍包含已删除 skill 引用: tdd/testing-anti-patterns"

Full test output:
{"pass": false, "errors": ["tdd-zh.md 合并内容不足：关键词命中 0/6，期望 >=5（缺失：['测试用例四要素', '前置条件', '输入参数', '操作序列', '预期输出', 'INSUFFICIENT_FUNDS']）", "tdd-zh.md 缺少四要素子节标题: ['### 前置条件', '### 输入参数', '### 操作序列', '### 预期输出']", "死代码文件未删除: internal/prompt/templates/skills/tc.md", "死代码文件未删除: internal/prompt/templates/skills/tdd.md", "死代码文件未删除: internal/prompt/templates/skills/tdd/testing-anti-patterns.md", "manager_test.go 仍包含已删除 skill 引用: \"tc\"", "manager_test.go 仍包含已删除 skill 引用: \"tdd\"", "manager_test.go 仍包含已删除 skill 引用: tdd/testing-anti-patterns\""]}
