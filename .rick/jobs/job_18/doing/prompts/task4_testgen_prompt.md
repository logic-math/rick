...
**Task ID**: task4
**Task Name**: 构建验证：build + dry-run 输出检查 + 回归测试
**Task Goal**: 确认 task1-3 所有改动编译通过，plan/easy 的 dry-run 或生成文件正确包含 grilling skill 路径（无未替换占位符），现有 go test 不引入新的 FAIL，plan_check 通过。
...
请创建测试脚本到: `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_18/doing/tests/task4.py`
...
- 脚本应该可以直接运行: `python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_18/doing/tests/task4.py`
...
```bash
python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_18/doing/tests/task4.py
```
