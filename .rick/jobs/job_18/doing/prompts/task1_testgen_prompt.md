...
**Task ID**: task1
**Task Name**: 创建 grilling skill 文件（templates/skills/grilling.md）
**Task Goal**: 在 `internal/prompt/templates/skills/` 下新建 `grilling.md`，内容包含 grilling 追问协议、终止条件（所有决策落实到具体代码路径或工具调用）、操作规范，供 plan 和 easy 模板通过 `{{grilling_skill_path}}` 变量注入使用。文件通过 embed.FS 的 `//go:embed templates/skills` 指令自动包含，无需修改 manager.go。
...
请创建测试脚本到: `/Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_18/doing/tests/task1.py`
...
- 脚本应该可以直接运行: `python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_18/doing/tests/task1.py`
...
```bash
python3 /Users/sunquan/ai_coding/CODING/rick/.rick/jobs/job_18/doing/tests/task1.py
```
