...
**成功标准**（全部满足时退出）：
- `/workdir/sunquan20/AI_CODING/rick/bin/rick tools learning_check job_37` pass
- `/workdir/sunquan20/rick-dev/.rick/jobs/job_37/learning/SUMMARY.md` 已写入，首行 `APPROVED: true`
...
**输出**：
- `/workdir/sunquan20/rick-dev/.rick/skills/{name}_skill/skill.md` — 新 skill 目录
- `/workdir/sunquan20/rick-dev/.rick/loops/{name}-loop.md` — 新 loop 文件
- `/workdir/sunquan20/rick-dev/.rick/jobs/job_37/learning/SUMMARY.md` — 执行总结
...
读取 `/workdir/sunquan20/rick-dev/.rick/jobs/job_37/doing/prompts/skill_gen_skill.md`，按其定义的格式（触发场景 / 预期效果 / 核心内容）从 runtime-trace 和 debug 中提取可复用技能：

- 每个 skill 创建目录 `/workdir/sunquan20/rick-dev/.rick/skills/{name}_skill/`
- 主文件写入 `/workdir/sunquan20/rick-dev/.rick/skills/{name}_skill/skill.md`
- 如有辅助脚本，写入同目录的 `.py` 文件

**parent 验收**：`ls /workdir/sunquan20/rick-dev/.rick/skills/` 确认目录已创建（如无可提取的 skill 则跳过）。
...
读取 `/workdir/sunquan20/rick-dev/.rick/jobs/job_37/doing/prompts/skill_gen_loop.md`，从 runtime-trace 和 debug 中识别 job 内反复出现的循环模式，按其定义的完整格式写入 `/workdir/sunquan20/rick-dev/.rick/loops/{name}-loop.md`：
...
**parent 验收**：`ls /workdir/sunquan20/rick-dev/.rick/loops/*.md` 确认文件已写入（如无可识别的 loop 则跳过）。
...
```
已生成：
- Skills: ls /workdir/sunquan20/rick-dev/.rick/skills/
- Loops:  ls /workdir/sunquan20/rick-dev/.rick/loops/
...
写入 `/workdir/sunquan20/rick-dev/.rick/jobs/job_37/learning/SUMMARY.md`：
...
**parent 验收**：`/workdir/sunquan20/rick-dev/.rick/jobs/job_37/learning/SUMMARY.md` 存在，首行为 `APPROVED: true`。
...
```bash
/workdir/sunquan20/AI_CODING/rick/bin/rick tools learning_check job_37
```
...
| SUMMARY.md 存在 | `ls /workdir/sunquan20/rick-dev/.rick/jobs/job_37/learning/SUMMARY.md` |
...
3. **skill 目录结构**：`/workdir/sunquan20/rick-dev/.rick/skills/{name}_skill/skill.md`
4. **loop 文件**：`/workdir/sunquan20/rick-dev/.rick/loops/{name}-loop.md`
5. **SUMMARY.md 写入 learning 目录**：`/workdir/sunquan20/rick-dev/.rick/jobs/job_37/learning/SUMMARY.md`
