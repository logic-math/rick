...
**先读**：`/workdir/sunquan20/AICODING/rick/.rick/draft/loops/loop_4/prompts/skill_exporter.md`
...
2. 读前序上下文：
   - /workdir/sunquan20/AICODING/rick/.rick/draft/loops/loop_4/briefs/*.md（所有步骤简报）
   - /workdir/sunquan20/AICODING/rick/.rick/draft/loops/loop_4/judgment.md（human 全部判断原话）
3. 阶段一：基于 human 判断生成 RFC 大纲（章节骨架），呈现 sense_loop → human 确认
4. 阶段二：human 确认大纲后，按 SENSE 思考方式填内容
5. 写入 /workdir/sunquan20/AICODING/rick/.rick/draft/rfc/rfc-[主题]-[日期].md
```
...
写入 `/workdir/sunquan20/AICODING/rick/.rick/draft/rfc/rfc-[主题]-[日期].md`。
...
| 文件 | 写入时机 | 内容 |
|------|---------|------|
| `/workdir/sunquan20/AICODING/rick/.rick/draft/rfc/rfc-[主题]-[日期].md` | 阶段二完成后 | 严格基于 human 确认结论 |
